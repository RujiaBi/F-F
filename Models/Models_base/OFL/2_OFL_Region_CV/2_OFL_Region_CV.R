...
params = c("cv_global", "cv", "nu_r", "mu_2ass", "mu_3ass" ,"mu_4ass", "mu_5ass", "mu_6ass", "mu_7ass", "mu_8ass",
           "loglik_2ass", "loglik_3ass", "loglik_4ass", "loglik_5ass", "loglik_6ass", "loglik_7ass", "loglik_8ass") 
# cv is a vector with length=region# (16)
# mu_2ass is a vector with length = stocknum_2ass 
# mu_3ass is a matrix with nrow = stocknum_3ass, ncol=tynum_3ass
# mu_4ass is a matrix with nrow = stocknum_4ass, ncol=tynum_4ass
# mu_5ass is a matrix with nrow = stocknum_5ass, ncol=tynum_5ass
# mu_6ass is a matrix with nrow = stocknum_6ass, ncol=tynum_6ass
# mu_7ass is a matrix with nrow = stocknum_7ass, ncol=tynum_7ass
# mu_8ass is a matrix with nrow = stocknum_8ass, ncol=tynum_8ass


# Run jags model
system.time(fit <- run.jags(model="2_OFL_Region_CV.txt", monitor=params, data=data, burnin=10000, sample=1000, adapt=10000, thin=20, n.chains=5, method = 'rjparallel'))
save(fit, file = '2_OFL_Region_CV_fit.RData')
fit0 = add.summary(fit)
save(fit0, file = '2_OFL_Region_CV_fit0.RData')
dic=extract(fit, 'dic')
save(dic, file='2_OFL_Region_CV_dic.RData')

load('2_OFL_Region_CV_dic.RData')
load('2_OFL_Region_CV_fit.RData')
