...
dat0 <- dat[!is.na(dat$B_by_Bmsy_recent) & !is.na(dat$B_by_Bmsy_old),]
...
params = c("cv_global", "cv_region", "cv", "constant", "nu_r", "mu_2ass", "mu_3ass" ,"mu_4ass", "mu_5ass", "mu_6ass", "mu_7ass", "mu_8ass",
           "loglik_2ass", "loglik_3ass", "loglik_4ass", "loglik_5ass", "loglik_6ass", "loglik_7ass", "loglik_8ass") 

# Run jags model
system.time(fit <- run.jags(model="4_B_Bmsy_Region_CV_constant.txt", monitor=params, data=data, burnin=20000, sample=1000, adapt=20000, thin=20, n.chains=5, method = 'rjparallel'))
save(fit, file = '4_B_Bmsy_Region_CV_constant_fit.RData')
fit0 = add.summary(fit)
save(fit0, file = '4_B_Bmsy_Region_CV_constant_fit0.RData')
dic=extract(fit, 'dic')
save(dic, file='4_B_Bmsy_Region_CV_constant_dic.RData')

load('4_B_Bmsy_Region_CV_constant_dic.RData')
load('4_B_Bmsy_Region_CV_constant_fit.RData')
