...
# Run jags model
system.time(fit <- run.jags(model="3_B_Region_Period_CV.txt", monitor=params, data=data, burnin=20000, sample=1000, adapt=20000, thin=20, n.chains=5, method = 'rjparallel'))
save(fit, file = '3_B_Region_Period_CV_fit.RData')
fit0 = add.summary(fit)
save(fit0, file = '3_B_Region_Period_CV_fit0.RData')
dic=extract(fit, 'dic')
save(dic, file='3_B_Region_Period_CV_dic.RData')

load('3_B_Region_Period_CV_dic.RData')
load('3_B_Region_Period_CV_fit.RData')
