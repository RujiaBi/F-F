...
params = c("cv", "loglik_2ass", "loglik_3ass", "loglik_4ass", "loglik_5ass", "loglik_6ass", "loglik_7ass", "loglik_8ass") 

# Run jags model
system.time(fit <- run.jags(model="1_B_Common_CV_mu.txt", monitor=params, data=data, burnin=20000, sample=1000, adapt=20000, thin=20, n.chains=5, method = 'rjparallel'))
save(fit, file = '1_B_Common_CV_mu_fit.RData')
dic=extract(fit, 'dic')
save(dic, file='1_B_Common_CV_mu_dic.RData')
